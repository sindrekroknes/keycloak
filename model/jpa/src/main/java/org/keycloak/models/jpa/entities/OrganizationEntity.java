...
@NamedQueries({
        ...
        @NamedQuery(name="getManagingGroupsByMember", query="select m.groupId from UserGroupMembershipEntity m join GroupEntity g on g.id = m.groupId where g.type = 1 and m.user.id = :userId and m.membershipType = 'MANAGED'"),
